...
def main():
    ...
    # production deploys will set a salt to get a contract address that
    salt = ""

    argobytes_atomic_trade_tx = accounts[0].deploy(ArgobytesAtomicTradeDeployer).deploy2(salt, KollateralInvokerAddress)

    argobytes_atomic_trade = ArgobytesAtomicTrade.at(argobytes_atomic_trade_tx.new_contracts[0])
