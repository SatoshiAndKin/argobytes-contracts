...
contract Weth9Action {
    ...
    function wrap_all_to(address weth, address to)
        external
        payable
    {
        if (to == address(0x0)) {
    ...
    function approved_unwrap_all_to(address weth, address payable to, uint256 amount)
        external
    {
        ...
        // send ETH to the next contract
        // TODO: use .call{value:...}(...)
        require(to.send(amount), "Weth9Action.approved_unwrap_all_to: send failed");
    }
}
